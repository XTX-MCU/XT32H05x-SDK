...
static void XT_I2cTcs34725_Write8(uint8_t reg, uint32_t value)
{
    ...
    if(HAL_I2C_Master_Transmit_IT(&hI2c1,(uint16_t)(DEVICE_TCS34725_ADDRESS),txbuff,length) != HAL_OK)
    {
...
static uint16_t XT_I2cTcs34725_Read8(uint8_t reg)
{
    ...
    HAL_I2C_Master_Transmit_IT(&hI2c1,(uint16_t)(DEVICE_TCS34725_ADDRESS),&txbuff,1);
    XT_I2c_Checksta(CB_I2C1_TXFNSH);	
    HAL_Delay(3); //ms		 	
    if(HAL_I2C_Master_Receive_IT(&hI2c1,(uint16_t)(DEVICE_TCS34725_ADDRESS),(uint8_t*)rxbuff,1) != HAL_OK)
    {
        if (HAL_I2C_GetError(&hI2c1) != HAL_I2C_ERROR_NONE)
        {
            Error_Handle();
...
static uint16_t XT_I2cTcs34725_Read16(uint8_t reg)
{
    ...
    XT_I2c_Checksta(CB_I2C1_TXFNSH);	
    HAL_Delay(3); //ms		
    if(HAL_I2C_Master_Receive_IT(&hI2c1,(uint16_t)(DEVICE_TCS34725_ADDRESS),(uint8_t*)rxbuff,2) != HAL_OK)
    {
        if (HAL_I2C_GetError(&hI2c1) != HAL_I2C_ERROR_NONE)
        {
            Error_Handle();

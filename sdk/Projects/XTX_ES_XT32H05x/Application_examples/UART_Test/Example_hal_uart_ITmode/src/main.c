...
static void SystemClock_Config(void)
{
  ...
  */
  ...
  RCC_OscInitStruct.PLL.PLLN = RCC_PLLN_DIV_FREQ_96M;
  RCC_OscInitStruct.PLL.PLLR = RCC_PLLR_DIV_FREQ_96M;
  ...
  */
  ...
  __HAL_RCC_GPIO_CONFIG(RCC_GPIOCLKSOURCE_HSI); // Enable source Clock:  GPIO source cloc
  __HAL_RCC_SET_GPIO_CLK_DIV(1);	
  __HAL_RCC_BR_CONFIG(RCC_BRCLKSOURCE_HSE);  // Enable source Clock:  UART Baudrate source clock

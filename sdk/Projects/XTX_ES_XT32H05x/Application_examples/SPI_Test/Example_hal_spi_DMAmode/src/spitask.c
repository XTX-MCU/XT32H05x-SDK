...
void XT_SPI2S_Init(void)
{
    ...
    hSpi2slaver.Init.ClockPhase    = SPI_PHASE_1EDGE;
...
void XT_Spi_Task(void)
{
    ...
    if(HAL_SPI_Transmit_DMA(&hSpi1master, (uint8_t *)aSpi1TxBuffer, sizeof(aSpi1TxBuffer))!=HAL_OK)
  	{
  	    Error_Handle();
  	}
    ...
    if(HAL_SPI_Receive_DMA(&hSpi1master, (uint8_t *)aRxBuffer, sizeof(aSpi1TxBuffer)-1)!=HAL_OK)
    {
        Error_Handle();
    }
    while (HAL_SPI_GetState(&hSpi1master) != HAL_SPI_STATE_READY);	
    ...
    if(HAL_SPI_TransmitReceive_DMA(&hSpi1master, (uint8_t *)aSpi1TxBuffer, (uint8_t *)aRxBuffer, sizeof(aSpi1TxBuffer))!=HAL_OK)
    {
        Error_Handle();
    }
    ...
    /*test spi2 as slaver transfer comunication*/	
  	if(HAL_SPI_Transmit_DMA(&hSpi2slaver, (uint8_t *)aSpi2TxBuffer, sizeof(aSpi2TxBuffer))!=HAL_OK)
    {
        Error_Handle();
    }
    ...
    if(HAL_SPI_Receive_DMA(&hSpi2slaver, (uint8_t *)aRxBuffer, sizeof(aSpi2TxBuffer)-1)!=HAL_OK)
    {
        Error_Handle();
    }
    while (HAL_SPI_GetState(&hSpi2slaver) != HAL_SPI_STATE_READY);

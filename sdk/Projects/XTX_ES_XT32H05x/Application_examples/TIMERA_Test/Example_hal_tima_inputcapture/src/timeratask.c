...
static uint32_t uwBurstRD_data[6] = {0};
/**
  * @}
  */
...
void XT_TIM1_Init(void)
{
    ...
    sConfigIC.ICPrescaler = TIM_ICPSC_DIV2;
    sConfigIC.ICFilter = 0;
    if (HAL_TIM_IC_ConfigChannel(&htima1, &sConfigIC, TIM_CHANNEL_2) != HAL_OK)
    {
      Error_Handler();
    }
    ...
    sConfigIC.ICFilter = 0;
    if (HAL_TIM_IC_ConfigChannel(&htima1, &sConfigIC, TIM_CHANNEL_3) != HAL_OK)
    {
      Error_Handler();
    }
    ...
    sConfigIC.ICPrescaler = TIM_ICPSC_DIV1;
    sConfigIC.ICFilter = 0;
    if (HAL_TIM_IC_ConfigChannel(&htima1, &sConfigIC, TIM_CHANNEL_4) != HAL_OK)
    {
      Error_Handler();
    }
}
...
static uint32_t XT_Captured_Calculater(TIM_HandleTypeDef *htim, uint32_t input_ch)
{
    ...
    if(u2CaptureStep == CAPTURE_STEP_READY)
    {
        ...
        uwICaptureValue1 = HAL_TIM_ReadCapturedValue(htim,  input_ch);
        u2CaptureStep = CAPTURE_STEP_START;
    }
    else if(u2CaptureStep == CAPTURE_STEP_START)
    {
        /* Get the 2nd Input Capture value */
        uwICaptureValue2 = HAL_TIM_ReadCapturedValue(htim, input_ch);
...
void XT_TIM1_Start(void )
{
    ...
    if (HAL_TIM_IC_Start_IT(&htima1, TIM_CHANNEL_3) != HAL_OK)
    {
    /*Error_Handler*/
    }
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if(HAL_TIM_GetActiveChannel(htim)== HAL_TIM_ACTIVE_CHANNEL_1 )
    {
    	...
    	XT_Captured_Calculater(htim, TIM_FLAG_CC1);
    }
